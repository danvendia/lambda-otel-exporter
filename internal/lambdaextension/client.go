...
// New returns a new Lambda Extensions API client
func New(baseURL string, extensionName string) *Client {
...
func (c *Client) Register(ctx context.Context) (*RegisterResponse, error) {
	...
	c.ExtensionID = res.Header.Get(extensionIdentifierHeader)
	if len(c.ExtensionID) == 0 {
		log.Println("No extension identifier returned in header")
	}
	return &resp, nil
}
